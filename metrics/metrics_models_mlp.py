...
def parse_args():
    parser = argparse.ArgumentParser(description="Train and evaluate RF model.")
...
if __name__ == '__main__':
    ...
    for nt in tqdm(range(1, 11)):
        ...
        if only_growth_rate:
            pipeline_path = os.path.join(args.model_save_path, f'pipeline_mlp_only_growth_rate_{nt}.pkl')
        else:
            pipeline_path = os.path.join(args.model_save_path, f'pipeline_mlp_{nt}.pkl')
    ...
    # Save metrics
    if only_growth_rate:
        save_metrics(pearsonr_list, args.metrics_save_path, 'pearsonr_list_mlp_only_growth_rate.txt')
        save_metrics(rmse_list, args.metrics_save_path, 'rmse_list_mlp_only_growth_rate.txt')
    else:
        save_metrics(pearsonr_list, args.metrics_save_path, 'pearsonr_list_mlp.txt')
        save_metrics(rmse_list, args.metrics_save_path, 'rmse_list_mlp.txt')
